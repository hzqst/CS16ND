...
void CFuncRotating::SpinUp(void)
{
	...
	if (abs((int)vecAVel.x) >= fabs((int)pev->movedir.x * pev->speed) && fabs((int)vecAVel.y) >= fabs((int)pev->movedir.y * pev->speed) && fabs((int)vecAVel.z) >= fabs((int)pev->movedir.z * pev->speed))
	{
